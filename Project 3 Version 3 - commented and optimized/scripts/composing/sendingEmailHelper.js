...
function callPostForSendingEmails(reqInboxJSON, reqSentItemsJSON) {
  $.post(
    SERVER_URL + "/storeEmailToInboxJSON",
    reqInboxJSON,
    runAfterStoringInbox
  ).fail(runIfFailStoring);
  $.post(
    SERVER_URL + "/storeEmailToSentJSON",
    reqSentItemsJSON,
    runAfterStoringSent
  ).fail(runIfFailStoring);

  function runAfterStoringInbox(data) {
    if (DEBUG) console.log("Successfully stored in the inbox");
  }

  function runAfterStoringSent(data) {
    if (DEBUG) console.log("Successfully stored in the sent");
  }

  function runIfFailStoring(err) {
    if (DEBUG) console.log("error in storing an email");
  }
}
